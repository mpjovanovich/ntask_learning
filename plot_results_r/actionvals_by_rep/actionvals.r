## Single task rep perseveration plotting in the case of X true tasks
## 2017 Mike Jovanovich

## Rscript actionvals.r 1 3 3 1
...
tasks_to_complete <- 100    # tasks completed before quitting
        ...
        # Plot the values learned by the ancillary rep value NN
            for( d in 1:ndims ) {
                for( f in 1:nfeatures ) {
                    ...
                    cat(sprintf('%.4f',nndot(convolve(current_ac,p[,rep_to_measure]),W_tr_anc)))
