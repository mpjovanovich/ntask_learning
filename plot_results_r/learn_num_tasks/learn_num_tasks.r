## Test to show learn number of tasks functionality
## 2017 Mike Jovanovich

## Rscript learn_num_tasks.r 1 5 5 r1_5_5

source("~/MTSU/Thesis/td_hrr/taskreps/hrr.r")
require(methods)
require(Matrix)
...
tasks_to_complete <- 500    # tasks completed before quitting
...
# Add/Remove thresholds - tuned for 3_3
tr_add_threshold <- c(replicate(10,.5))      # This should be higher than tr_remove_threshold
tr_remove_threshold <- c(replicate(10,0))   # This should be lower than tr_add_threshold
...
# custom output
vals <- matrix(nrow=max_trials,ncol=5)
ts_threshold_vals <- rep('NA',max_trials)
task_switch_trial <- 0

output_dir <- args[4]
result_files <- c('r1','r2','r3','r4','r5')
ts_threshold_file <- 'ts_threshold'
task_switch_file <- 'task_switch'
...
while( total_trials < max_trials && tasks_complete < tasks_to_complete*2 )
{
    ...
    # This is for the custom output
    for( r in 1:num_reps)
        vals[total_trials+1,r] <- V_tr[r]
    ts_threshold_vals[total_trials+1] <- task_switch_threshold
    if( tasks_complete == tasks_to_complete && task_switch_trial == 0 )
        task_switch_trial <- total_trials + 1
...
# without seed suffix
for( r in 1:num_reps )
    write(vals[,r],file=file.path(output_dir,result_files[r]),sep=',',ncolumns=max_trials,append=FALSE)
write(ts_threshold_vals,file=file.path(output_dir,ts_threshold_file),sep=',',ncolumns=max_trials,append=FALSE)
write(task_switch_trial,file=file.path(output_dir,task_switch_file),sep=',',ncolumns=1,append=FALSE)

# with seed suffix
#for( r in 1:num_reps )
    #write(vals[,r],file=file.path(output_dir,paste(result_files[r],'_',args[1],sep='')),sep=',',ncolumns=max_trials,append=FALSE)
#write(ts_threshold_vals,file=file.path(output_dir,paste(ts_threshold_file,'_',args[1],sep='')),sep=',',ncolumns=max_trials,append=FALSE)
#write(task_switch_trial,file=file.path(output_dir,paste(task_switch_file,'_',args[1],sep='')),sep=',',ncolumns=1,append=FALSE)
