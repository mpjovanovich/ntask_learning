## Static task rep values test
## 2017 Mike Jovanovich

## Rscript static_rep_taskvals.r 1 3 3 3
...
tasks_to_complete <- 500    # 1/2 of tasks completed before quitting
...
while( total_trials < max_trials && tasks_complete < tasks_to_complete*2 )
{
    ...
    if( consecutive_correct == trials_per_task ) {
        ...
        # custom output
        submoves[tasks_complete] <- (total_trials/tasks_complete)-trials_per_task
...
# custom output
for( r in 1:num_reps )
    write(vals[,r],file=paste(result_files[r],'_',args[1],sep=''),sep=',',ncolumns=max_trials,append=FALSE)
write(submoves,file=paste(submove_file,'_',args[1],sep=''),sep=',',ncolumns=max_trials,append=FALSE)
